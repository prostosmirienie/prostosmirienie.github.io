...
The three you'll most likely encounter:
...
On most modern systems, typing `vi` opens `vim` — it's just an alias. So when you see `vi`
in documentation, they almost always mean `vim`.

Install vim if it's missing:
...
> `:w` will fail if you don't have write permissions on the file — in that case `:q!` is
...
These basics cover ~50% of everyday use cases. For some people, that's enough for a lifetime.
